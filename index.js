...
app.get('/admin', sessionValidation, adminAuthorization, async (req,res) => {
	if (req.session.authenticated){
		...
		const promote = async (user) => {
            db.user.updateOne({username: user}, {$set: {user_type:"admin"}});
        };
		const demote = async (user) => {
            db.user.updateOne({username: user}, {$set: {user_type:"user"}});
        };
		console.log("test");
    	res.render("admin", {users: result, promote: promote, demote: demote});
	} else {
		req.session.errorMessage = "not logged in, please login";
		res.redirect("/login");
